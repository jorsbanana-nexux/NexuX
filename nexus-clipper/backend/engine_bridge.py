"""Bridge compatibility agents to the V8.0 engine modules.
...
# V8.0: vision_quality is now in engine/ — no more legacy path injection

# V8.0 engine imports (primary) — these use lazy imports for cv2/mediapipe
...
def detect_scene_changes(path, start: float = 0.0, end=None, threshold: float = 30.0, sample_fps: float = 2.0):
    """Legacy-compatible wrapper over V8.0 engine.detect_scene_changes."""
...
def detect_face_subjects(path, start: float = 0.0, end=None, sample_fps: float = 3.0):
    """Legacy-compatible wrapper over V8.0 engine.analyze_faces."""
...
def visual_quality(path, start: float = 0.0, end=None, sample_fps: float = 2.0):
    ...
    try:
        from engine.vision_quality import visual_quality as _vq  # noqa: E402
...
def inspect_render(path, expected_width=None, expected_height=None, min_duration=None, max_duration=None):
    """Lazy-loaded from legacy vision_quality module (requires cv2)."""
    try:
        from engine.vision_quality import inspect_render as _ir  # noqa: E402
