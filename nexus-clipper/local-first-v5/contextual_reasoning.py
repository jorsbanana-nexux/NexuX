...
def enrich_world_with_context(world: AnalysisWorld, *, radius: float = 18.0) -> AnalysisWorld:
    """Attach context-window and promise→payoff verification to each world candidate."""
    world.validate()
    ...
    editorial["contextual_verification"] = {
        "enabled": True,
    ...
    return build_analysis_world(
        ...
        audio_profiles=world.audio.get("profiles", {}),
        scenes=world.vision.get("scenes", []),
        subjects=world.vision.get("subjects", []),
        candidates=enriched,
        editorial=editorial,
...
def contextual_score(candidate: dict[str, Any]) -> float:
    ...
    quality = float(assessment.get("context_integrity", 0.0))
    match = float(assessment.get("promise_payoff_match", 0.0))
    risk = float(assessment.get("premature_cut_risk", 0.0))
