...
def _style_overrides(req: Any) -> dict[str, Any]:
    return {
        "font": req.font,
        "size": req.font_size,
...
def _camera_path(video: Path, clip: dict[str, Any], timeline: Any, req: Any) -> list[Any]:
...
def render_with_spec(
    ...
    media = job.get("meta")
    if not media:
        raise RuntimeError("Render service requires persisted media metadata")
    ...
    camera_points = _camera_path(video, clip, timeline, req)
    layout = dynamic_layout_plan(
        aspect_ratio=req.aspect_ratio,
        genre=str(clip.get("genre", "general")),
        face_tracking=req.face_tracking,
        auto_zoom=req.auto_zoom,
    )
    layout_bias = 0.015 if layout.get("layout") == "kinetic" else 0.0
    edl_graph = ffmpeg_filter_for_timeline(timeline)[0]
    filter_complex = build_final_filter(
        edl_graph, camera_points, ass, source_w, source_h, spec, layout_bias=layout_bias
    )
    run_ffmpeg(
        video, output, filter_complex,
        spec=spec, job_id=job.get("job_id"),
    ...
    return {
        ...
        "camera": {
            "face_tracking": req.face_tracking,
            "auto_zoom": req.auto_zoom,
            "points": path_to_dict(camera_points),
