...
def test_world_gets_contextual_evidence():
    world = build_analysis_world(job_id="ctx-1", transcript=transcript(), candidates=[{"id": "c1", "start": 0.0, "end": 18.0, "duration": 18.0, "text": "Why did we almost fail?"}], editorial={"intent": {"objective": "storytelling"}}, provenance={"world": "analysis_world:v2"}, confidence={"world": 0.8})
...
def test_selection_preserves_contextual_lineage():
    world = build_analysis_world(job_id="ctx-2", transcript=transcript(), candidates=[
    ...
    ], editorial={"intent": {"objective": "storytelling"}}, provenance={"world": "analysis_world:v2"}, confidence={"world": 0.8})
