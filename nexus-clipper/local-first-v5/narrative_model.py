...
def assess_narrative(candidate: Mapping[str, Any], transcript: Mapping[str, Any] | None = None) -> NarrativeAssessment:
    ...
    segs = [dict(s) for s in transcript.get("segments", []) or [] if float(s.get("end", 0.0)) > start and float(s.get("start", 0.0)) < end]
    ...
    promise_strength = 0.55 if promise else 0.20
    tension_curve = _clamp(0.25 + 0.45*tension + 0.20*revelation + 0.10*(len(beats) >= 3))
    revelation_strength = _clamp(0.15 + 0.75*revelation)
    payoff_strength = _clamp(0.15 + 0.75*payoff + 0.10*bool(_C.search(last)))
    ...
    premature = _clamp(0.45*continuity + 0.35*(1-payoff_strength) + 0.20*(1-context))
    unresolved = _clamp((0.70 if question and not payoff else 0) + 0.20*(1-context))
