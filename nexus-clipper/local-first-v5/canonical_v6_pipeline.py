...
async def run_generation(job_id: str, req: engine.GenerateRequest) -> None:
    ...
    try:
        ...
        source = {"type": "youtube", "url": req.youtube_url, "max_height": 1080}
        ...
        transcript, recon_source = await asyncio.to_thread(_reconnaissance, job_dir, req.youtube_url, req.language, job_id)
        engine._set(job, stage="candidate_generation", progress=28, transcript=transcript, retrieval={"strategy": "caption-first-targeted", "recon_source": recon_source})
        ...
        if getattr(req, "remove_fillers_pauses", True):
            filler_cuts = detect_filler_segments(transcript.get("segments", []), min_pause=float(getattr(req, "pause_threshold", 0.42)))
        ...
        for idx, candidate in enumerate(render_candidates):
            if engine.CANCEL_FLAGS.get(job_id):
                engine._set(job, status="cancelled", stage="cancelled")
                return
            engine._set(job, stage=f"retrieving clip {idx + 1}/{len(render_candidates)}", progress=40 + int(8 * idx / max(1, len(render_candidates))))
            ...
            local_candidate = await asyncio.to_thread(_rerank_single_candidate, local_candidate, local_transcript, scenes, req.target_duration)
            local_candidate, _ = apply_editorial_intelligence([local_candidate], prompt=getattr(req, "clip_prompt", None), genre=decision.genre)
            timeline = await asyncio.to_thread(engine.build_timeline, video, local_transcript, local_candidate)
            subject_samples = await asyncio.to_thread(engine.detect_face_subjects, video, float(local_candidate["start"]), float(local_candidate["end"]))
            ...
            if getattr(req, "voice_over", False):
                ...
                voiceover_path = engine.OUTPUTS / f"{job_id}_clip_{idx + 1:02d}_voiceover.mp3"
            ...
            info = await asyncio.to_thread(engine._render_with_spec, video, {**job, "job_id": job_id, "transcript": local_transcript, "meta": media}, local_candidate, output, timeline, req, voiceover_path)
            ...
            render_meta.append({
                ...
                "dynamic_layout": dynamic_layout_plan(aspect_ratio=req.aspect_ratio, genre=decision.genre, face_tracking=req.face_tracking, auto_zoom=req.auto_zoom),
                "retrieval": retrieval,
                "voiceover": str(voiceover_path) if voiceover_path else None,
            })
            engine._set(job, progress=50 + int(40 * (idx + 1) / len(render_candidates)), stage=f"rendering {idx + 1}/{len(render_candidates)}", render_meta=render_meta)
...
def _rerank_single_candidate(candidate: dict[str, Any], transcript: dict[str, Any], scenes: list[dict[str, Any]], target_duration: int) -> dict[str, Any]:
    ranked = engine.rerank_candidates([candidate], scene_boundaries=scenes, target_duration=float(target_duration), limit=1, video=None, transcript=transcript)
