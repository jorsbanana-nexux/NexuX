...
async def run_generation(job_id: str, req: GenerateRequest, runtime: CanonicalRuntime | None = None) -> None:
    ...
    try:
        ...
        runtime.set_job(job, status="processing", stage="reconnaissance", progress=5, source={"type":"youtube","url":req.youtube_url,"max_height":1080,"metadata":source_probe.metadata}, job_dir=str(job_dir), retrieval={"strategy":"caption-first-targeted"})

        transcript, recon_source = await asyncio.to_thread(_reconnaissance, runtime, job_dir, req.youtube_url, req.language, job_id)
        runtime.set_job(job, stage="candidate_generation", progress=24, transcript=transcript, retrieval={"strategy":"caption-first-targeted","recon_source":recon_source})
        ...
        initial_world, initial_world_path = build_and_persist_world(runtime.jobs_dir, job_id=job_id, media=source_probe.metadata, transcript=transcript, candidates=candidates[:min(80,max(req.clip_count*12,40))], editorial={"prompt":getattr(req,"clip_prompt",None),"genre":getattr(req,"genre","auto"),"target_duration":req.target_duration,"decision":decision.to_dict(),"stage":"pre_retrieval"}, provenance={"world":"analysis_world:v2","transcript":transcript.get("source","unknown")}, confidence=_world_confidence(candidates,{},transcript))
        ...
        world,world_path=build_and_persist_world(runtime.jobs_dir,job_id=job_id,media=source_probe.metadata,transcript=transcript,audio_profiles=audio_profiles,scenes=world_scenes,subjects=world_subjects,candidates=enriched_candidates,editorial={"prompt":getattr(req,"clip_prompt",None),"genre":getattr(req,"genre","auto"),"target_duration":req.target_duration,"decision":decision.to_dict(),"stage":"multimodal_editorial"},provenance={"world":"analysis_world:v2","transcript":transcript.get("source","unknown"),"audio":"audio_intelligence","vision":"vision_service"},confidence=_world_confidence(enriched_candidates,audio_profiles,transcript))
        ...
        for idx,candidate in enumerate(final_candidates):
            ...
            render_meta.append({"candidate_id":local_candidate["id"],"timeline":timeline.to_dict(),"render":info,"editorial_rank":candidate.get("editorial_rank"),"editorial_signals":candidate.get("editorial_signals"),"editorial_evidence":candidate.get("editorial_evidence"),"analysis_world":candidate.get("analysis_world"),"virality":local_candidate.get("virality_score"),"prompt_relevance":local_candidate.get("prompt_relevance"),"genre":local_candidate.get("genre",decision.genre),"dynamic_layout":dynamic_layout_plan(aspect_ratio=req.aspect_ratio,genre=decision.genre,face_tracking=req.face_tracking,auto_zoom=req.auto_zoom),"retrieval":retrieval,"audio_profile":audio_profile,"audio_signals":audio_features,"voiceover":str(voiceover_path) if voiceover_path else None})
