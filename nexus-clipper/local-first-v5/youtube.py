...
def download_youtube(url: str, job_dir: Path, max_height: int = 1080, job_id: str | None = None) -> tuple[Path, dict[str, Any]]:
    ...
    cmd = [
        ...
        "--retries", "8", "--fragment-retries", "8", "--continue", "--part",
        "-f", fmt, "-o", str(output), url,
    ]
    result = supervised_run(cmd, key=f"download:{job_id}" if job_id else None, timeout=int(os.getenv("DOWNLOAD_TIMEOUT_SECONDS", "14400")))
    if result.returncode != 0:
