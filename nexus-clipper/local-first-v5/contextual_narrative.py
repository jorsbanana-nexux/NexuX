...
class ContextualNarrativeAssessment:
    ...
    def to_dict(self) -> dict[str, Any]:
        return {"schema_version": self.schema_version, "candidate_id": self.candidate_id, "before": [x.to_dict() for x in self.before], "candidate": self.candidate.to_dict(), "after": [x.to_dict() for x in self.after], "promise_text": self.promise_text, "payoff_text": self.payoff_text, "promise_payoff_match": round(self.promise_payoff_match, 3), "context_gain": round(self.context_gain, 3), "dependency_score": round(self.dependency_score, 3), "context_integrity": round(self.context_integrity, 3), "premature_cut_risk": round(self.premature_cut_risk, 3), "unresolved_question_risk": round(self.unresolved_question_risk, 3), "recommendation": self.recommendation, "confidence": round(self.confidence, 3), "reasons": list(self.reasons)}
...
def _segment_windows(transcript: Mapping[str, Any], candidate: Mapping[str, Any], radius: float = 18.0) -> tuple[list[ContextWindow], ContextWindow, list[ContextWindow]]:
    ...
    for seg in transcript.get("segments", []) or []:
        ...
        if e <= start: before.append(ContextWindow("before", s, e, text, max(0.0, start - e)))
        elif s >= end: after.append(ContextWindow("after", s, e, text, max(0.0, s - end)))
        else: inside.append(ContextWindow("candidate", s, e, text, 0.0))
    if inside:
...
def verify_context(candidate: Mapping[str, Any], transcript: Mapping[str, Any] | None = None, *, radius: float = 18.0) -> ContextualNarrativeAssessment:
    ...
    if promise_payoff_match >= 0.55 and context_integrity >= 0.72 and premature < 0.45: recommendation = "KEEP"
    elif context_integrity >= 0.55: recommendation = "EXTEND" if payoff else "REFINE"
