...
app = FastAPI(title="NexuX Local-First V5", version="5.0.0")
...
def render(video: Path, job: dict[str, Any], clip: dict[str, Any], output: Path) -> None:
    ...
    vf = "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,ass='{}'".format(str(ass).replace("\\", "/").replace(":", "\\:"))
    cmd = ["ffmpeg", "-y", "-ss", str(max(0, clip["start"])), "-i", str(video), "-t", str(duration), "-vf", vf,
           "-c:v", "libx264", "-preset", "medium", "-crf", "20", "-c:a", "aac", "-b:a", "192k", "-movflags", "+faststart", str(output)]
...
def health() -> dict[str, Any]:
    return {"status": "ok", "ffmpeg": shutil.which("ffmpeg") is not None, "ffprobe": shutil.which("ffprobe") is not None, "whisper_model": WHISPER_MODEL}
...
async def upload(file: UploadFile = File(...)):
    ...
    job = {"job_id": job_id, "status": "uploaded", "video_path": str(target), "meta": meta}
    save_job(job_id, job)
    return job
